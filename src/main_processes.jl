...
function parse_args(args::Vector{String})
    ...
    return solver_names, input_file, race_mode, timeout
end

"""
    race_solvers_multiprocess(instance_file, solver_names, timeout) -> (solution, time, winning_solver)

Run multiple solvers in parallel as separate processes.
Returns the first solution found along with the solver that found it.
Each solver runs in complete isolation - no shared memory or threading issues.
"""
...
"""
    run_single_solver_process(input_file, solver_name, timeout) -> (solution, time, solver_name)

Run a single solver as a separate process.
"""
...
function main(args::Vector{String})
    solver_names, input_file, race_mode, timeout = parse_args(args)
    ...
    if isnothing(input_file)
        ...
        println("  julia main_multiprocess.jl --race --solver dpll --solver cdcl_vsids test.cnf")
        println("  julia main_multiprocess.jl --race --timeout 600 --solver cdcl_vsids --solver cdcl_vsids_luby test.cnf")
        return
    end
    ...
    try
        ...
        sol, elapsed, winning_solver = if race_mode && length(solver_names) > 1
            ...
            run_single_solver_process(input_file, solver_names[1], timeout)
        end
