...
# ──────────────────────────────────────────────────────────────────────────────
# Solver state (VSIDS + Luby restarts)
# ──────────────────────────────────────────────────────────────────────────────
...
function reduce_db!(s::Solver)
    ...
    for ci in (s.num_original_clauses + 1):length(s.clauses)
        ...
        clause = s.clauses[ci]
        if length(clause) <= 3; continue; end
        locked = false
    ...
    n_delete = div(length(candidates), 10)
    for k in 1:n_delete
...
# ──────────────────────────────────────────────────────────────────────────────
# Entry point (VSIDS + Luby restarts)
# ──────────────────────────────────────────────────────────────────────────────
...
function cdcl_solve(instance::SATInstance)::Union{Dict{Int, Bool}, Nothing}
    ...
    while true
        ...
        if conflict != 0
            ...
            num_learned = length(solver.clauses) - solver.num_original_clauses
            if solver.num_conflicts >= next_reduce && num_learned >= 10_000
                reduce_db!(solver)
                next_reduce += reduce_interval
            elseif solver.num_conflicts >= next_reduce
                next_reduce += reduce_interval
            end
