...
mutable struct DPLL_TwoWatched
    ...
    function DPLL_TwoWatched(instance::SATInstance)
...
@inline function is_satisfied(solver::DPLL_TwoWatched, lit::Int)::Bool
    var = abs(lit)
...
@inline function is_falsified(solver::DPLL_TwoWatched, lit::Int)::Bool
    var = abs(lit)
    val = solver.assignment[var]
    if val == 0
        return false
...
@inline function is_unassigned(solver::DPLL_TwoWatched, lit::Int)::Bool
    return solver.assignment[abs(lit)] == 0
end

# Assign a variable
function assign!(solver::DPLL_TwoWatched, var::Int, value::Bool)
...
function backtrack!(solver::DPLL_TwoWatched, level::Int)
...
function propagate!(solver::DPLL_TwoWatched, lit::Int)::Bool
...
# Find a unit clause
function find_unit_clause(solver::DPLL_TwoWatched)::Union{Int, Nothing}
...
function all_satisfied(solver::DPLL_TwoWatched)::Bool
    for clause in solver.clauses
        clause_sat = false
        for lit in clause.literals
            if is_satisfied(solver, lit)
...
function pick_branching_variable(solver::DPLL_TwoWatched)::Union{Int, Nothing}
...
function solve!(solver::DPLL_TwoWatched)::Bool
    # Unit propagation
    while true
...
function get_solution(solver::DPLL_TwoWatched)::Dict{Int, Bool}
...
# Main entry point
function run_dpll_optimized(instance::SATInstance)::Union{Dict{Int, Bool}, Nothing}
    try
        solver = DPLL_TwoWatched(instance)
        if solve!(solver)
